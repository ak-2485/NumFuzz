...
module Options = struct
  ...
  let default_components = [Lexer; Parser; TypeChecker]

  let components = ref default_components

  let comp_enabled comp = List.mem comp !components
  ...
  let debug_default = {
    components   = [General; Lexer; Parser; TypeChecker];
    level        = 2;
    unicode      = true;
    pr_ann       = true;
    pr_level     = 8;
...
module Error = struct
  ...
  (* Printing levels:
     ...
     5: Print everything, including detailed stuff about type eq 
        and unification, context merging, etc...
  ...
  let message level component fi =
    ...
    (level_to_string level) (comp_to_string component) FileInfo.pp_fileinfo fi;
	Format.kfprintf (fun ppf -> Format.fprintf ppf "@]@]@.") Format.err_formatter
      end
    else
      Format.ifprintf Format.err_formatter

  (* Note that the eprintf here will be executed *)
  let error_msg comp fi =
    let cont _ =
      Format.eprintf "@]@.";
      raise (Exit 1)                in
    Format.eprintf "@[%s %s %a: " 
      (level_to_string 0) (comp_to_string comp) FileInfo.pp_fileinfo fi;
